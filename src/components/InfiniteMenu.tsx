...
void main() {
    ...
    vec3 tint = uColors[itemIndex];
    outColor.rgb = mix(tint, outColor.rgb, outColor.a);
    outColor.a = max(0.5, outColor.a);
    outColor.a *= vAlpha;
    outColor.a = max(0.15, outColor.a);
}`;
...
class Geometry {
  ...
  subdivide(divisions = 1) {
    ...
    for (let div = 0; div < divisions; ++div) {
      ...
      f.forEach((face, ndx) => {
        const mAB = this.getMidPoint(face.a, face.b, midPointCache), mBC = this.getMidPoint(face.b, face.c, midPointCache), mCA = this.getMidPoint(face.c, face.a, midPointCache);
        const i = ndx * 4;
        newFaces[i + 0] = new Face(face.a, mAB, mCA); newFaces[i + 1] = new Face(face.b, mBC, mAB); newFaces[i + 2] = new Face(face.c, mCA, mBC); newFaces[i + 3] = new Face(mAB, mBC, mCA);
      });
      f = newFaces;
    }
...
class DiscGeometry extends Geometry {
  constructor(steps = 4, radius = 1) {
    ...
    for (let i = 0; i < steps; ++i) { const x = Math.cos(alpha * i), y = Math.sin(alpha * i); this.addVertex(radius * x, radius * y, 0); this.lastVertex.uv[0] = x * 0.5 + 0.5; this.lastVertex.uv[1] = y * 0.5 + 0.5; if (i > 0) this.addFace(0, i, i + 1); }
...
function resizeCanvasToDisplaySize(canvas: HTMLCanvasElement) {
  const dpr = Math.min(2, window.devicePixelRatio); const displayWidth = Math.round(canvas.clientWidth * dpr), displayHeight = Math.round(canvas.clientHeight * dpr);
  const needResize = canvas.width !== displayWidth || canvas.height !== displayHeight;
  if (needResize) { canvas.width = displayWidth; canvas.height = displayHeight; } return needResize;
}

function makeBuffer(gl: WebGL2RenderingContext, sizeOrData: Float32Array | number, usage: number) { const buf = gl.createBuffer()!; gl.bindBuffer(gl.ARRAY_BUFFER, buf); gl.bufferData(gl.ARRAY_BUFFER, sizeOrData, usage); gl.bindBuffer(gl.ARRAY_BUFFER, null); return buf; }
...
class ArcballControl {
  ...
  snapDirection = vec3.fromValues(0, 0, -1); snapTargetDirection?: vec3;
  EPSILON = 0.1; IDENTITY_QUAT = quat.create();
  pointerPos = vec2.create(); previousPointerPos = vec2.create();
  _rotationVelocity = 0; _combinedQuat = quat.create();
  canvas: HTMLCanvasElement; updateCallback: (deltaTime: number) => void;
  ...
  update(deltaTime: number, targetFrameDuration = 16) {
    ...
    if (this.isPointerDown) {
      ...
      if (this.snapTargetDirection) { const SNAPPING_INTENSITY = 0.2; const a = this.snapTargetDirection, b = this.snapDirection; const sqrDist = vec3.squaredDistance(a, b); const distanceFactor = Math.max(0.1, 1 - sqrDist * 10); angleFactor *= SNAPPING_INTENSITY * distanceFactor; this.quatFromVectors(a, b, snapRotation, angleFactor); }
    }
  ...
  quatFromVectors(a: vec3, b: vec3, out: quat, angleFactor = 1) { const axis = vec3.cross(vec3.create(), a, b); vec3.normalize(axis, axis); const d = Math.max(-1, Math.min(1, vec3.dot(a, b))); const angle = Math.acos(d) * angleFactor; quat.setAxisAngle(out, axis, angle); }
  #project(pos: vec2) { const r = 2, w = this.canvas.clientWidth, h = this.canvas.clientHeight, s = Math.max(w, h) - 1; const x = (2 * pos[0] - w - 1) / s, y = (2 * pos[1] - h - 1) / s; const xySq = x * x + y * y, rSq = r * r; let z = 0; if (xySq <= rSq / 2.0) z = Math.sqrt(rSq - xySq); else z = rSq / Math.sqrt(xySq); return vec3.fromValues(-x, y, z); }
}

class InfiniteGridMenu {
  TARGET_FRAME_DURATION = 1000 / 60; SPHERE_RADIUS = 2;
  #time = 0; #deltaTime = 0; #deltaFrames = 0; #frames = 0;
  camera: any; nearestVertexIndex: number | null = null; smoothRotationVelocity = 0; scaleFactor = 1.0; movementActive = false;
  canvas!: HTMLCanvasElement; items: any[]; onActiveItemChange: (i: number) => void; onMovementChange: (m: boolean) => void;
  gl!: WebGL2RenderingContext; viewportSize!: vec2; drawBufferSize!: vec2;
  discProgram!: WebGLProgram; discLocations!: Record<string, any>;
  discGeo!: DiscGeometry; discBuffers!: any; discVAO!: WebGLVertexArrayObject;
  icoGeo!: IcosahedronGeometry; instancePositions!: vec3[]; DISC_INSTANCE_COUNT!: number;
  worldMatrix!: mat4; tex!: WebGLTexture; atlasSize!: number;
  control!: ArcballControl; discInstances!: { matricesArray: Float32Array; matrices: Float32Array[]; buffer: WebGLBuffer };

  constructor(canvas: HTMLCanvasElement, items: any[], onActiveItemChange: (i: number) => void, onMovementChange: (m: boolean) => void, onInit: ((sk: InfiniteGridMenu) => void) | null = null, scale = 1.0) {
  ...
  resize() { this.viewportSize = vec2.set(this.viewportSize || vec2.create(), this.canvas.clientWidth, this.canvas.clientHeight); const gl = this.gl; if (resizeCanvasToDisplaySize(gl.canvas)) gl.viewport(0, 0, gl.drawingBufferWidth, gl.drawingBufferHeight); this.#updateProjectionMatrix(gl); }
  ...
  // Public: rotate sphere to face a specific item
  goToIndex(idx: number) {
    const targetIdx = idx % Math.max(1, this.items.length);
    const vertexIdx = targetIdx % this.instancePositions.length;
    const targetPos = this.instancePositions[vertexIdx];
    const dir = vec3.normalize(vec3.create(), targetPos);
    const m = mat4.create(); mat4.targetTo(m, [0, 0, 0], dir, [0, 1, 0]); mat4.invert(m, m);
    mat4.getRotation(this.control.orientation, m); quat.normalize(this.control.orientation, this.control.orientation);
    this.control.snapTargetDirection = dir;
    this.onActiveItemChange(targetIdx);
  }

  #init(onInit: ((sk: InfiniteGridMenu) => void) | null) {
    const gl = this.canvas.getContext('webgl2', { antialias: true, alpha: false }); if (!gl) throw new Error('No WebGL 2 context!');
    this.gl = gl; this.viewportSize = vec2.fromValues(this.canvas.clientWidth, this.canvas.clientHeight); this.drawBufferSize = vec2.clone(this.viewportSize);
    ...
    this.discLocations = {
      ...
      uColors: gl.getUniformLocation(this.discProgram, 'uColors')
    };
  ...
  #initTexture() {
    ...
    Promise.all(this.items.map(item => new Promise<HTMLImageElement>(resolve => { const img = new Image(); img.onload = () => resolve(img); img.onerror = () => resolve(img); img.src = item.image; }))).then(images => {
  ...
  #initDiscInstances(count: number) {
    ...
    const bytesPerMatrix = 16 * 4; for (let j = 0; j < 4; ++j) { const loc = this.discLocations.aInstanceMatrix + j; gl.enableVertexAttribArray(loc); gl.vertexAttribPointer(loc, 4, gl.FLOAT, false, bytesPerMatrix, j * 4 * 4); gl.vertexAttribDivisor(loc, 1); }
  ...
  #animate(deltaTime: number) {
    ...
    positions.forEach((p, ndx) => {
      ...
      const matrix = mat4.create(); mat4.multiply(matrix, matrix, mat4.fromTranslation(mat4.create(), vec3.negate(vec3.create(), p)));
  ...
  #render() {
    ...
    gl.clearColor(0.05, 0.08, 0.03, 1); gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
    ...
    // Per-disc colors
    const colors = [[0,1,0.25],[0.55,1,0.08],[0,0.8,0.15],[0,1,0.7],[0,0.8,0.8]];
    gl.uniform3fv(this.discLocations.uColors, new Float32Array(colors.flat()));
  ...
  #getVertexWorldPosition(index: number) { const nearestVertexPos = this.instancePositions[index]; return vec3.transformQuat(vec3.create(), nearestVertexPos, this.control.orientation); }
}
...
export default function InfiniteMenu({ items = [], scale = 1.0 }: { items: MenuItem[]; scale?: number }) {
  ...
  useEffect(() => {
    const canvas = canvasRef.current; let sketch: InfiniteGridMenu;
    const handleActiveItem = (index: number) => { const i = index % items.length; setActiveItem(items[i]); setActiveIdx(i); };
    if (canvas) { sketch = new InfiniteGridMenu(canvas, items.length ? items : defaultItems, handleActiveItem, setIsMoving, sk => sk.run(), scale); sketchRef.current = sketch; }
    ...
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {activeItem && (
        <>
          ...
          {activeItem.link && (
            <a href={activeItem.link} className={`action-button ${isMoving ? 'inactive' : 'active'}`}
      ...
      <div className="im-progress">
        {items.map((_, i) => (
          <div key={i} className={`im-dot ${i === activeIdx ? 'im-dot-active' : ''}`}
            onClick={() => sketchRef.current?.goToIndex(i)} style={{ cursor: 'pointer' }} />
        ))}
      </div>
    </div>
  );
